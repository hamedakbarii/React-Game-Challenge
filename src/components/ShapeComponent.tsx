...
const ShapeComponent: React.FC = () => {
  const getRandomImage = () => {
    const isSymmetrical = Math.random() < 0.5; // 50% chance of being symmetrical
    const folder = isSymmetrical ? "symmetrical" : "asymmetrical";

    // Assuming you have a known set of image filenames, you can also dynamically fetch them from the server
    const imageFilenames = ["1.svg", "2.svg", "3.svg , 4.svg , 5.svg"];
    const randomIndex1 = Math.floor(Math.random() * imageFilenames.length);
    const randomIndex2 = (randomIndex1 + 1) % imageFilenames.length; // Ensures the second image is different
